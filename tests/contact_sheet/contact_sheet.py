...
def main(output_file, image_dir=None, file_list=None, no_crop=None, img_size=0, heic_arg=None, shuffle=None):
    if heic_arg:
        heic_image_paths = [os.path.join(image_dir, file) for file in os.listdir(image_dir) if
                            file.lower().endswith(".heic")]
        for heic_img in heic_image_paths:
            heic_converter(path=heic_img, out_path=str(
                image_dir + "/" + os.path.basename(heic_img).strip(".heic") + "." + str(heic_arg)), format=heic_arg)
    ...
    if file_list is not None:
        ...
        image_paths = [os.path.join(image_dir, file) for file in os.listdir(image_dir) if
                       file.lower().endswith(accepted_extensions)]
    else:
